...
export function LandingPage() {
  const { user } = useAuth();
  const [currentUniverse, setCurrentUniverse] = useState<'blackhole' | 'solar' | 'matrix' | 'simulation' | 'infotopo' | 'multilayer' | 'hyperdim'>('blackhole');
  ...
  const handleEnterVortex = (selectedUniverse?: string) => {
    ...
    if (selectedUniverse === 'spaces-manager') {
      ...
      console.log('⚠️ No universe specified, staying at entry portal');
      // Stay at blackhole entry portal instead of auto-navigating
    }
  };
