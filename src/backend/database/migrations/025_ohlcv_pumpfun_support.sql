...
-- Update existing pump.fun pools
UPDATE ohlcv_data 
SET pool_type = 'bonding_curve'
WHERE pool_address IN (
  SELECT pool_address FROM token_pools WHERE dex = 'pump.fun'
);

-- Update existing raydium pools  
UPDATE ohlcv_data
SET pool_type = 'dex'
WHERE pool_address IN (
  SELECT pool_address FROM token_pools WHERE dex = 'raydium'
);
