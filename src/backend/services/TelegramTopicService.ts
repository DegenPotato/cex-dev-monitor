...
export class TelegramTopicService {
   ...
   */
  async discoverForumTopics(userId: number, chatId: string): Promise<any> {
    ...
    try {
      ...
      for (const topic of result.topics || []) {
        ...
        `, [userId, chatId, topicData.topicId]) as any;
   ...
   */
  async getCachedTopicsWithAnalytics(userId: number, chatId: string): Promise<any[]> {
   ...
   */
  async updateTopicUserFilter(
    userId: number, 
    chatId: string, 
    topicId: string,
    monitoredUserIds: string[] | null,
   ...
   */
  async getTopicUserFilter(userId: number, chatId: string, topicId: string): Promise<any> {
  ...
  async trackTopicPerformance(
    ...
    `, [userId, chatId, topicId, today]) as any;
   ...
   */
  async getBestPerformingTopics(userId: number, limit: number = 10): Promise<any[]> {
