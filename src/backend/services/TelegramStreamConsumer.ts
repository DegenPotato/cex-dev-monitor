...
interface Metrics {
  ...
  avgGoToRedisLatency: number;
  avgRedisToWebSocketLatency: number;
  avgDatabaseBatchLatency: number;
...
export class TelegramStreamConsumer extends EventEmitter {
  ...
  private goToRedisLatencies: number[] = [];
  private redisToWsLatencies: number[] = [];
  private dbBatchLatencies: number[] = [];
  ...
  async startConsuming() {
    if (this.isConsuming || !this.redis) return;
   ...
   */
  private async processMessage(_messageId: string, data: any) {
    try {
      const wsStartTime = Date.now();
      const detection: Detection = JSON.parse(data.data);
   ...
   */
  private broadcastToWebSocket(detection: Detection) {
    if (!this.wss) return;
   ...
   */
  private trackLatency(type: 'goToRedis' | 'redisToWs' | 'dbBatch', latency: number) {
    const arrays = {
