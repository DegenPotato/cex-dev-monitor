...
export class TechnicalIndicatorCalculator {
  ...
  private async calculateAllIndicators() {
    try {
      ...
      const combinations = await queryAll<{ mint_address: string; timeframe: string }>(`
   ...
   */
  private async storeIndicators(candle: OHLCVCandle, indicators: TechnicalIndicators) {
    try {
      await execute(`
   ...
   */
  async getIndicators(mintAddress: string, timeframe: string, limit: number = 100) {
    return queryAll(`
   ...
   */
  async getLatestIndicators(mintAddress: string, timeframe: string) {
    return queryOne(`
