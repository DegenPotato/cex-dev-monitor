...
export class TelegramEntityCache {
  private entityCache = new Map<string, {
  ...
  private readonly REFRESH_INTERVAL = 20 * 60 * 1000; // 20 minutes
  ...
  async initialize() {
    ...
    const destinations = await queryAll<{ target_chat_ids: string }>(
      `SELECT DISTINCT target_chat_ids FROM telegram_forwarding_rules WHERE is_active = 1`
    );
    ...
    for (const dest of destinations) {
      try {
        ...
        targets.forEach((t: string) => uniqueTargets.add(t));
      } catch (e) {
        // Skip invalid JSON
      }
    }
   ...
   */
  async preloadEntities(client: any, targetIds: string[]) {
   ...
   */
  getCachedEntity(clientUserId: string, targetId: string) {
  ...
  private startPeriodicRefresh() {
    this.refreshTimer = setInterval(async () => {
      ...
      const targetsToRefresh = new Set<string>();
      for (const dest of destinations) {
        try {
          const targets = JSON.parse(dest.target_chat_ids || '[]');
          targets.forEach((t: string) => targetsToRefresh.add(t));
        } catch (e) {
          // Skip invalid JSON
        }
      }
