...
interface Token {
  mint_address: string;
  timestamp: number; // Creation timestamp (in milliseconds from DB)
}
...
export class OHLCVCollectorV2 {
  ...
  private readonly MAX_CANDLES = 1000; // GeckoTerminal max per request
  private readonly BATCH_SIZE = 100; // Database batch insert size
    ...
    if (candles.length === 0) {
      ...
      if (fetchMode === 'initial') {
        await this.updateProgress(
          ...
          nowUnix,
          nowUnix,
          true // Mark complete since no data exists
        );
      }
      return;
    }
