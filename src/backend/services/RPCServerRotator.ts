...
export class RPCServerRotator {
  ...
  private hostHeader = 'api.mainnet-beta.solana.com';
  
  // Stats tracking
  private serverStats: Map<string, { requests: number; failures: number }> = new Map();
  
  // Per-server rate limiting (safety ceiling: never exceed 90 req/10s per server)
  private readonly MAX_REQUESTS_PER_10S = 90;
  private serverRequestTimestamps: Map<string, number[]> = new Map();
   ...
   */
  private cleanOldTimestamps(server: string): void {
   ...
   */
  private trackRequest(server: string): void {
    const timestamps = this.serverRequestTimestamps.get(server) || [];
   ...
   */
  markFailure(server: string): void {
    const stats = this.serverStats.get(server);
    if (stats) {
      stats.failures++;
    }
