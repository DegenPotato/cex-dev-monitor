...
export class SolanaMonitor extends EventEmitter {
  ...
  private batchConnection: RateLimitedConnection;
  private subscriptions: Map<string, number> = new Map();
  private thresholdSol: number;
  private maxThresholdSol: number;
  private walletAnalyzer: WalletAnalyzer;
  private devWalletAnalyzer: DevWalletAnalyzer;
  ...
  constructor() {
    ...
    ConfigProvider.get('threshold_sol').then(value => {
      if (value) this.thresholdSol = parseFloat(value);
    }).catch(console.error);
    
    ConfigProvider.get('max_threshold_sol').then(value => {
      if (value) this.maxThresholdSol = parseFloat(value);
    }).catch(console.error);
  }
  ...
  private async checkRecentTransactions(walletAddress: string, isRealTime: boolean = false): Promise<void> {
    try {
      const publicKey = new PublicKey(walletAddress);
  ...
  private async processTransaction(
    ...
    const monitoredIndex = accountKeys.findIndex(
      (key: any) => key.pubkey.toBase58() === monitoredAddress
    );

    if (monitoredIndex === -1) return;
    ...
    if (balanceChange >= -this.thresholdSol) return;
    ...
    let maxRecipient: { address: string; change: number } | null = null;
    
    console.log(`\n📊 Transaction ${signature.slice(0, 8)}... analysis:`);
    for (let i = 0; i < accountKeys.length; i++) {
      if (i === monitoredIndex) continue;
  ...
  private async analyzeWalletAsync(walletAddress: string, source: string, amount: number): Promise<void> {
    try {
      ...
      this.checkDevHistoryAsync(walletAddress).catch((err: any) => {
  ...
  private async checkDevHistoryAsync(walletAddress: string): Promise<void> {
    try {
  ...
  updateMaxThreshold(newMaxThreshold: number): void {
  ...
  getWalletAnalyzer(): WalletAnalyzer {
    return this.walletAnalyzer;
  }

  getDevWalletAnalyzer(): DevWalletAnalyzer {
