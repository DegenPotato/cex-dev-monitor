...
interface DetectedEvent {
    ...
    program_id?: string;
    log_message?: string;
    raw_tx?: ParsedTransactionWithMeta;
    timestamp: number;
}
...
export class SolanaEventDetector extends EventEmitter {
    ...
    private subscriptions: Map<number, Campaign> = new Map();
    ...
    private async handleLogNotification(params: any): Promise<void> {
        const { result } = params;
        const { value } = result;
    ...
    private async checkCampaignTriggers(campaign: Campaign, signature: string, logs: string[]): Promise<void> {
        const triggerNodes = campaign.nodes?.filter(n => n.node_type === 'trigger') || [];
    ...
    private async checkTransferTriggers(): Promise<void> {
    ...
    private async checkMonitoringTargets(): Promise<void> {
    ...
    private async checkTransactionForMonitors(
        wallet: string, 
        ...
        for (const target of targets) {
            ...
            const hasMonitoredProgram = monitor_config.programs_to_watch?.some(
                (pid: string) => programIds.includes(pid)
            );

            if (hasMonitoredProgram) {
                // Check for specific events (simplified - in production would parse instructions)
                const hasInitializeMint = tx.meta?.logMessages?.some(
                    log => log.includes('InitializeMint')
                );
                const hasMintTo = tx.meta?.logMessages?.some(
                    log => log.includes('MintTo')
                );
    ...
    private cleanExpiredTargets(): void {
        const now = Date.now();
    ...
    async handleTriggerFired(campaign: Campaign, node: CampaignNode, event: DetectedEvent): Promise<void> {
        console.log(`🎯 Trigger fired for campaign ${campaign.name}: ${node.node_id}`);
        
        // Check deduplication
        const dedupKey = `${event.wallet || event.signature}`;
        const isDuplicate = await this.checkDeduplication(campaign.id!, dedupKey, node.config as TriggerConfig);
    ...
    private async checkDeduplication(
        ...
        try {
            ...
            await execute(
                `INSERT INTO campaign_deduplication (campaign_id, dedup_key, expires_at)
                 VALUES (?, ?, ?)`,
                [campaignId, dedupKey, expiresAt]
            );
            
            // Clean old records
            await execute(
                `DELETE FROM campaign_deduplication 
                 WHERE expires_at < ?`,
    ...
    private async handleMonitorDetection(
        campaignId: number, 
        instanceId: number, 
        detection: any
    ): Promise<void> {
        ...
        // Log the event
        await execute(
            `INSERT INTO campaign_events (instance_id, campaign_id, node_id, event_type, event_data, tx_signature, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [instanceId, campaignId, 'monitor', 'monitor_detected', JSON.stringify(detection), 
    ...
    async getNetCreditedLamports(wallet: string, signature: string): Promise<number> {
        try {
    ...
    async checkWalletForTriggers(wallet: string): Promise<void> {
        try {
            const pubkey = new PublicKey(wallet);
            const signatures = await this.connection.getSignaturesForAddress(pubkey, {
                limit: 10
...
// Singleton instance
let solanaEventDetector: SolanaEventDetector | null = null;

export function getSolanaEventDetector(): SolanaEventDetector {
