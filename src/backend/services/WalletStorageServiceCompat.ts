...
export class WalletStorageServiceCompat {
   ...
   */
  async getSchemaVersion(): Promise<'old' | 'new'> {
    try {
  ...
  async getWalletKeypair(walletId: number, userId: number): Promise<Keypair> {
    ...
    let wallet: any;
    let privateKeyField: string;
    
    if (schema === 'old') {
      wallet = await queryOne(
        'SELECT encrypted_private_key, wallet_address FROM trading_wallets WHERE id = ? AND user_id = ?',
        [walletId, userId]
      );
      privateKeyField = wallet?.encrypted_private_key;
    } else {
      wallet = await queryOne(
        'SELECT private_key, public_key FROM trading_wallets WHERE id = ? AND user_id = ? AND is_deleted = 0',
        [walletId, userId]
      );
  ...
  async createWallet(userId: number, walletName?: string): Promise<{
    ...
    if (schema === 'old') {
      ...
      await execute(
        `INSERT INTO trading_wallets (user_id, wallet_address, encrypted_private_key)
         VALUES (?, ?, ?)`,
  ...
  async importWallet(userId: number, privateKey: string, walletName?: string): Promise<{
    ...
    if (schema === 'old') {
      await execute(
        `INSERT OR REPLACE INTO trading_wallets (user_id, wallet_address, encrypted_private_key)
         VALUES (?, ?, ?)`,
        [userId, publicKey, encryptedPrivateKey]
      );
    } else {
      await execute(
        `INSERT OR REPLACE INTO trading_wallets (user_id, wallet_name, public_key, private_key, sol_balance, is_deleted)
         VALUES (?, ?, ?, ?, 0, 0)`,
        [userId, walletName || `Imported ${Date.now()}`, publicKey, encryptedPrivateKey]
      );
    }
    ...
    const wallet = await queryOne(
      schema === 'old'
        ...
        : 'SELECT id FROM trading_wallets WHERE user_id = ? AND public_key = ?',
      [userId, publicKey]
    ) as { id: number } | undefined;
  ...
  async deleteWallet(walletId: number, userId: number): Promise<void> {
    ...
    if (schema === 'old') {
      ...
      await execute(
        'DELETE FROM trading_wallets WHERE id = ? AND user_id = ?',
        [walletId, userId]
      );
    } else {
      // Soft delete for new schema
      await execute(
        'UPDATE trading_wallets SET is_deleted = 1 WHERE id = ? AND user_id = ?',
        [walletId, userId]
      );
    }
  }
  ...
  async exportWallet(walletId: number, userId: number): Promise<{
    ...
    if (schema === 'old') {
      wallet = await queryOne(
        'SELECT wallet_address, encrypted_private_key FROM trading_wallets WHERE id = ? AND user_id = ?',
        [walletId, userId]
      );
    } else {
      wallet = await queryOne(
        'SELECT public_key, private_key FROM trading_wallets WHERE id = ? AND user_id = ? AND is_deleted = 0',
        [walletId, userId]
      );
    }
