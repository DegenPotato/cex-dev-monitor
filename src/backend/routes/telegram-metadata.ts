...
router.post('/metadata/:chatId/fetch', authService.requireSecureAuth(), async (req, res) => {
  try {
    ...
    const telegramService = (global as any).telegramClientService;
    if (!telegramService) {
...
router.get('/metadata/:chatId', authService.requireSecureAuth(), async (req, res) => {
  try {
    ...
    const metadata = await queryOne(
      'SELECT * FROM telegram_chat_metadata WHERE user_id = ? AND chat_id = ?',
      [userId, chatId]
    );
...
router.get('/metadata', authService.requireSecureAuth(), async (req, res) => {
  try {
    ...
    const metadata = await queryAll(
      `SELECT m.*, c.chat_name as monitored_chat_name, c.is_active
...
router.post('/metadata/bulk-fetch', authService.requireSecureAuth(), async (req, res) => {
  try {
    ...
    const results = await Promise.allSettled(
      chatIds.map(chatId => telegramService.fetchAndStoreChatMetadata(userId, chatId))
    );
...
router.get('/metadata/admin-chats', authService.requireSecureAuth(), async (req, res) => {
  try {
    ...
    res.json({
      ...
      chats: adminChats,
      breakdown: {
        creator: adminChats.filter((c: any) => c.is_creator).length,
        admin: adminChats.filter((c: any) => c.is_admin && !c.is_creator).length
      }
    });
  } catch (error: any) {
