 ...
 */
router.get('/api/indicators/:mintAddress', authService.requireSecureAuth(), async (req: Request, res: Response) => {
  try {
    ...
    const data = await queryAll<IndicatorData>(query, params);
 ...
 */
router.get('/api/indicators/:mintAddress/timeframes', authService.requireSecureAuth(), async (req: Request, res: Response) => {
  try {
    const { mintAddress } = req.params;
    
    const timeframes = await queryAll<{ timeframe: string; count: number }>(`
      SELECT 
        timeframe,
        COUNT(*) as count
 ...
 */
router.get('/api/indicators/:mintAddress/latest', authService.requireSecureAuth(), async (req: Request, res: Response) => {
  try {
    const { mintAddress } = req.params;
    const { timeframe = '1m' } = req.query;
    
    const latest = await queryOne<IndicatorData>(`
      SELECT 
        o.timestamp,
        o.open,
        o.high,
...
// Helper functions
function determineEMATrend(data: IndicatorData): string {
  if (!data.close || !data.ema_21 || !data.ema_50) return 'unknown';
...
function determineBollingerPosition(data: IndicatorData): string {
  ...
  if (price > lower) return 'above_lower';
  return 'below_lower';
}
