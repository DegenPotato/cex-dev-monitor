...
class ReferralTrackingProvider {
    constructor() {
        this.maxLevels = 10; // Maximum referral chain depth
    }
    /**
     * Process referral attribution for new user registration
     */
    async processReferralAttribution(newUserId, referralCode) {
        try {
            ...
            const referrer = await queryOne('SELECT * FROM users WHERE referral_code = ?', [referralCode]);
            if (!referrer) {
            ...
            const newUser = await queryOne('SELECT * FROM users WHERE id = ?', [newUserId]);
            if (!newUser) {
                return {
                    success: false,
                    attributed: false,
            ...
            await execute('UPDATE users SET referred_by = ? WHERE id = ?', [referrer.id, newUserId]);
            // Update referrer's total referral count
            await execute('UPDATE users SET total_referrals = total_referrals + 1 WHERE id = ?', [referrer.id]);
     ...
     */
    async getReferralChain(userId, maxLevels = this.maxLevels) {
        try {
            const chain = [];
            ...
            while (currentUserId && level <= maxLevels) {
                ...
                const user = await queryOne('SELECT * FROM users WHERE id = ?', [currentUserId]);
                if (!user || !user.referred_by) {
                    break; // End of chain
                }
                // Get the referrer
                const referrer = await queryOne('SELECT * FROM users WHERE id = ?', [user.referred_by]);
                if (!referrer) {
                    break;
                }
                chain.push({
     ...
     */
    async distributeReferralCommissions(userId, amount, activityType = 'transaction', metadata = {}) {
        try {
            ...
            for (const chainLevel of referralChain) {
                try {
                    ...
                    await execute(`INSERT INTO reward_transactions 
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                    ...
                    await execute('UPDATE users SET total_commission_earned = total_commission_earned + ? WHERE id = ?', [commission, chainLevel.userId]);
     ...
     */
    async calculateCommission(level, amount) {
        try {
            const config = await queryOne('SELECT * FROM referral_config WHERE tier_level = ? AND is_active = 1', [level]);
            if (!config) {
    ...
    async getReferralStats(userId) {
        try {
            ...
            const directReferrals = await queryAll('SELECT id, username, wallet_address, created_at FROM users WHERE referred_by = ?', [userId]);
            // Get total commission earned
            const commissionSum = await queryOne(`SELECT SUM(amount) as total FROM reward_transactions 
         WHERE user_id = ? AND transaction_type = 'referral_commission' AND status = 'completed'`, [userId]);
    ...
    async validateReferralCode(code) {
        try {
            const referrer = await queryOne('SELECT id, username, wallet_address, referral_code FROM users WHERE referral_code = ?', [code]);
            if (!referrer) {
                return { valid: false };
            }
            return {
