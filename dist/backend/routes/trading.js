...
router.post('/api/trading/wallets/:walletId/withdraw', authService.requireSecureAuth(), async (req, res) => {
    try {
        ...
        const user = await queryOne('SELECT wallet_address FROM users WHERE id = ?', [userId]);
        ...
        await execute(`INSERT INTO trading_transactions (user_id, wallet_id, tx_type, signature, amount_out, status, created_at)
       VALUES (?, ?, 'withdraw', ?, ?, 'completed', strftime('%s', 'now'))`, [userId, walletId, signature, amount]);
        res.json({
            success: true,
            signature,
            amount,
...
router.post('/api/trading/buy', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const { walletAddress, tokenMint, amount, slippageBps, priorityLevel, jitoTip } = req.body;
        if (!tokenMint || !amount) {
...
router.post('/api/trading/sell', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const { walletAddress, tokenMint, amount, percentage, slippageBps, priorityLevel, jitoTip } = req.body;
...
router.post('/api/trading/transfer', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const { walletAddress, tokenMint, destination, amount, priorityLevel } = req.body;
 ...
 */
router.get('/api/trading/history', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/api/trading/wallets/:walletId/tokens', authService.requireSecureAuth(), async (req, res) => {
    try {
        ...
        try {
            ...
            wallet = await queryOne('SELECT * FROM trading_wallets WHERE id = ? AND user_id = ? AND is_deleted = 0', [walletId, userId]);
        }
        catch (e) {
            // Fall back to old schema (no is_deleted column)
            wallet = await queryOne('SELECT * FROM trading_wallets WHERE id = ? AND user_id = ?', [walletId, userId]);
        }
        if (!wallet) {
            return res.status(404).json({ error: 'Wallet not found' });
        }
...
router.get('/api/trading/portfolio/stats', authService.requireSecureAuth(), async (req, res) => {
    try {
        ...
        try {
            // Try new schema first
            wallets = await queryAll('SELECT * FROM trading_wallets WHERE user_id = ? AND is_deleted = 0', [userId]);
        }
        catch (e) {
            // Fall back to old schema
            wallets = await queryAll('SELECT id, user_id, wallet_address as public_key, 0 as sol_balance FROM trading_wallets WHERE user_id = ?', [userId]);
        }
        ...
        const totalSOL = wallets.reduce((sum, w) => sum + (w.sol_balance || 0), 0);
      ...
      `, walletIds);
        ...
        const totalTokenValue = holdings.reduce((sum, h) => sum + (h.total_value || 0), 0);
...
router.get('/api/trading/config', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const config = await queryOne('SELECT * FROM trading_api_keys WHERE user_id = ?', [userId]);
        // Don't send encrypted keys to frontend
        if (config) {
            delete config.helius_api_key_encrypted;
            delete config.jito_api_key_encrypted;
            delete config.jupiter_api_key_encrypted;
        }
...
router.post('/api/trading/config', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const { heliusApiKey, jitoApiKey, jupiterApiKey, customRpcUrl, customWsUrl, useMonetizedRpc, maxPriorityFeeLamports, autoJitoTips } = req.body;
        ...
        const existing = await queryOne('SELECT id FROM trading_api_keys WHERE user_id = ?', [userId]);
        if (existing) {
            // Update
            const setClause = Object.keys(updates).map(k => `${k} = ?`).join(', ');
            await queryOne(`UPDATE trading_api_keys SET ${setClause} WHERE user_id = ?`, [...Object.values(updates), userId]);
        }
        else {
            // Insert
            ...
            await queryOne(`INSERT INTO trading_api_keys (${keys.join(', ')}) VALUES (${placeholders})`, Object.values(updates));
        }
        res.json({ success: true });
    }
    catch (error) {
