 ...
 */
router.get('/api/user/data-summary', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        // Count all user data
        const telegramAccounts = await queryOne('SELECT COUNT(*) as count FROM telegram_user_accounts WHERE user_id = ?', [userId]);
        const telegramBots = await queryOne('SELECT COUNT(*) as count FROM telegram_bot_accounts WHERE user_id = ?', [userId]);
        const monitoredChats = await queryOne('SELECT COUNT(*) as count FROM telegram_monitored_chats WHERE user_id = ?', [userId]);
        const detectedContracts = await queryOne('SELECT COUNT(*) as count FROM telegram_detected_contracts WHERE user_id = ?', [userId]);
        // Get last activity
        const lastActivity = await queryOne('SELECT last_connected_at FROM telegram_user_accounts WHERE user_id = ? ORDER BY last_connected_at DESC LIMIT 1', [userId]);
        res.json({
            userId,
            summary: {
 ...
 */
router.delete('/api/user/data/:type', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        switch (dataType) {
            ...
            case 'monitored-chats':
                const result1 = await queryOne('SELECT COUNT(*) as count FROM telegram_monitored_chats WHERE user_id = ?', [userId]);
            ...
            case 'detected-contracts':
                const result2 = await queryOne('SELECT COUNT(*) as count FROM telegram_detected_contracts WHERE user_id = ?', [userId]);
 ...
 */
router.delete('/api/user/data/all', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const count1 = await queryOne('SELECT COUNT(*) as count FROM telegram_user_accounts WHERE user_id = ?', [userId]);
        summary.telegramAccounts = count1?.count || 0;
        const count2 = await queryOne('SELECT COUNT(*) as count FROM telegram_bot_accounts WHERE user_id = ?', [userId]);
        summary.telegramBots = count2?.count || 0;
        const count3 = await queryOne('SELECT COUNT(*) as count FROM telegram_monitored_chats WHERE user_id = ?', [userId]);
        summary.monitoredChats = count3?.count || 0;
        const count4 = await queryOne('SELECT COUNT(*) as count FROM telegram_detected_contracts WHERE user_id = ?', [userId]);
 ...
 */
router.get('/api/user/data/export', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const { queryAll } = await import('../database/helpers.js');
        // Fetch all user data
        const telegramAccounts = await queryAll('SELECT * FROM telegram_user_accounts WHERE user_id = ?', [userId]);
        const telegramBots = await queryAll('SELECT * FROM telegram_bot_accounts WHERE user_id = ?', [userId]);
        const monitoredChats = await queryAll('SELECT * FROM telegram_monitored_chats WHERE user_id = ?', [userId]);
        const detectedContracts = await queryAll('SELECT * FROM telegram_detected_contracts WHERE user_id = ?', [userId]);
        const exportData = {
