...
router.get('/', async (req, res) => {
    try {
        const { source = 'all', sort = 'newest', timeframe = '24h', verified = 'false', traded = 'false', limit = 100 } = req.query;
...
router.get('/with-pricing', async (req, res) => {
    try {
        ...
        const tokens = await tokenRegistrySync.getTokensWithPricing(parseInt(limit), parseInt(offset));
        res.json(tokens);
    }
    catch (error) {
    ...
    `);
        ...
        bySource.forEach(row => {
            if (row.first_source_type === 'telegram') {
                ...
                sourceCounts[row.first_source_type] = row.count;
            }
        });
