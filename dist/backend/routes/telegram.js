...
export function createTelegramRoutes() {
    ...
    router.post('/user-account/start-auth', authService.requireSecureAuth(), async (req, res) => {
        try {
            ...
            const result = await telegramClientService.startAuth(userId, account.apiId, account.apiHash, account.phoneNumber);
            res.json(result);
        }
        catch (error) {
                    ...
                    (async () => {
                        ...
                        for (let i = 0; i < chats.length; i++) {
                            ...
                            try {
                                ...
                                failCount++;
                                console.error(`   ⚠️  Failed to fetch metadata for ${chat.chatId}:`, err.message);
                            }
    ...
    router.get('/all-chats', authService.requireSecureAuth(), async (req, res) => {
        try {
            ...
            const storedMetadata = await telegramService.getChatMetadata(userId);
            // Enrich chats with metadata
            const enrichedChats = chats.map((chat) => {
                const stored = storedMetadata?.find((m) => m.chat_id === chat.chatId);
                if (stored) {
                    return {
                        ...chat,
    ...
    router.post('/chats/:chatId/topics/:topicId/user-filter', authService.requireSecureAuth(), async (req, res) => {
        try {
            ...
            const { telegramTopicService } = await import('../services/TelegramTopicService.js');
            await telegramTopicService.updateTopicUserFilter(userId, chatId, topicId, monitoredUserIds, excludedUserIds);
            res.json({
                success: true,
    ...
    router.get('/chat-config/:chatId', authService.requireSecureAuth(), async (req, res) => {
        try {
            const userId = req.user.id;
            const { chatId } = req.params;
            const config = await queryOne(`SELECT * FROM telegram_chat_configs WHERE chat_id = ? AND user_id = ?`, [chatId, userId]);
            if (!config) {
    ...
    router.post('/chat-config', authService.requireSecureAuth(), async (req, res) => {
        try {
            const userId = req.user.id;
            const { chatId, duplicateStrategy, backlogScanDepth, backlogTimeLimit, minTimeBetweenDuplicates } = req.body;
