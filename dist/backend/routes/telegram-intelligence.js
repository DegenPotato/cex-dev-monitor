...
router.post('/api/telegram/track-caller', authService.requireSecureAuth(), async (req, res) => {
    try {
        ...
        const existing = await queryOne('SELECT id FROM telegram_callers WHERE user_id = ? AND telegram_user_id = ?', [userId, telegramUserId]);
        if (existing) {
...
router.post('/api/telegram/record-call', authService.requireSecureAuth(), async (req, res) => {
    try {
        const { callerId, chatId, messageId, contractAddress, tokenSymbol, tokenName, callTimestamp, callType, callMessage, priceAtCall, mcapAtCall, confidenceScore } = req.body;
        const userId = req.user.id;
        const result = await execute(`
    ...
    `, [userId, callerId, chatId, messageId, contractAddress,
        ...
        await execute('UPDATE telegram_callers SET total_calls = total_calls + 1, last_call_date = ? WHERE id = ?', [callTimestamp, callerId]);
...
router.post('/api/telegram/update-call-performance', authService.requireSecureAuth(), async (req, res) => {
    try {
        const { callId, athPrice, athTimestamp, currentPrice, currentMcap, volume24h, holderCount, isRugpull, isHoneypot } = req.body;
        const call = await queryOne('SELECT price_at_call, mcap_at_call, call_timestamp FROM telegram_token_calls WHERE id = ?', [callId]);
        if (!call) {
    ...
    `, [athPrice, call.mcap_at_call * athMultiplier, athTimestamp, athMultiplier,
        ...
        const callerCalls = await queryAll('SELECT ath_multiplier, time_to_ath, is_successful, volume_24h FROM telegram_token_calls WHERE caller_id = (SELECT caller_id FROM telegram_token_calls WHERE id = ?)', [callId]);
...
router.post('/api/telegram/calculate-reputation', authService.requireSecureAuth(), async (req, res) => {
    try {
        ...
        const caller = await queryOne('SELECT * FROM telegram_callers WHERE id = ?', [callerId]);
        if (!caller) {
        ...
        if (caller.is_verified)
            reputationScore += 5;
        // Check for rug pulls
        const rugpulls = await queryOne('SELECT COUNT(*) as count FROM telegram_token_calls WHERE caller_id = ? AND is_rugpull = 1', [callerId]);
        ...
        if (reputationScore >= 80)
            ...
            trustLevel = 'scammer';
        await execute('UPDATE telegram_callers SET reputation_score = ?, trust_level = ?, updated_at = ? WHERE id = ?', [Math.max(0, Math.min(100, reputationScore)), trustLevel, Date.now(), callerId]);
