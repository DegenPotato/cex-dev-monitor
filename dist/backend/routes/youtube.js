...
router.post('/oauth/link', authService.requireSecureAuth(), async (req, res) => {
    try {
...
router.get('/oauth/status', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.post('/oauth/unlink', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/preferences', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const db = getDatabase();
    ...
    `);
        ...
        const preferences = user?.youtube_preferences
            ? JSON.parse(user.youtube_preferences)
...
router.get('/playlists', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
    ...
    `);
        ...
        const formatted = playlists.map((row) => ({
            id: row[0],
            name: row[1],
...
router.post('/playlists', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
...
// Delete playlist
router.delete('/playlists/:id', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
...
// Update play count
router.post('/playlists/:id/play', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const playlistId = parseInt(req.params.id);
        runQuery(`
...
router.post('/history', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/history', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
