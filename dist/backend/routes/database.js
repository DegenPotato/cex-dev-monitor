...
router.get('/tables/:tableName/data', async (req, res) => {
    try {
        ...
        // Get total count
        const countResult = await queryOne(`SELECT COUNT(*) as count FROM ${tableName} ${whereClause}`, params);
        // Get paginated data
        const data = await queryAll(`SELECT * FROM ${tableName} ${whereClause} LIMIT ? OFFSET ?`, [...params, parseInt(limit), offset]);
        res.json({
            data,
            pagination: {
                page: parseInt(page),
...
router.put('/tables/:tableName/rows', async (req, res) => {
    try {
        ...
        await execute(`UPDATE ${tableName} SET ${setClause} WHERE ${whereClause}`, [...values, ...whereValues]);
...
router.delete('/tables/:tableName/rows', async (req, res) => {
    try {
        ...
        const whereClause = whereFields.map(f => `${f} = ?`).join(' AND ');
        const whereValues = whereFields.map(f => where[f]);
        await execute(`DELETE FROM ${tableName} WHERE ${whereClause}`, whereValues);
