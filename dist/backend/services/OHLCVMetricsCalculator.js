...
export class OHLCVMetricsCalculator {
    ...
    async calculateAllMetrics() {
        try {
            ...
            const tokensWithData = await queryAll(`
      ...
      `);
          ...
          `, [token.mint_address]);
             ...
             WHERE mint_address = ?`, [metrics.starting_mcap, finalAthMcap, metrics.current_mcap, Date.now(), token.mint_address]);
                    updated++;
                }
            }
            if (updated > 0) {
     ...
     */
    async calculateTokenMetrics(mintAddress) {
        try {
            // Get oldest candle (launch price) - use 1m timeframe for accuracy
            const oldestCandle = await queryOne(`
      ...
      `, [mintAddress]);
            ...
            const latestCandle = await queryOne(`
      ...
      `, [mintAddress]);
            ...
            const totalSupply = tokenData?.total_supply || 1000000000;
