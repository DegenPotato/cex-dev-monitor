...
export class OHLCVScheduler {
     ...
     */
    async getNextBatch(maxItems = 10) {
        const now = Date.now();
    ...
    `);
        for (const token of tokens) {
            ...
            if (token.telegram_mentions > 100)
                priority += 3;
            else if (token.telegram_mentions > 10)
                priority += 2;
            else if (token.telegram_mentions > 0)
                priority += 1;
            // High market cap = higher priority
            if (token.current_mcap > 10000000)
                priority += 3;
            else if (token.current_mcap > 1000000)
                priority += 2;
            else if (token.current_mcap > 100000)
                priority += 1;
     ...
     */
    async markCompleted(mintAddress, poolAddress, timeframe, success) {
