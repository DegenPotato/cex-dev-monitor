...
let Api;
let StringSession;
let NewMessage; // Used in message handler
...
export class TelegramClientService extends EventEmitter {
    constructor() {
        ...
        this.activeClients = new Map(); // TelegramClient instances (key: userId for users, 'bot_userId' for bots)
        this.metadataRefreshInterval = null;
        this.unmonitoredLogThrottle = {}; // Track last log time per chat
        this.encryptionKey = process.env.TELEGRAM_ENCRYPTION_KEY || 'default-key-change-in-production';
        this.solanaConnection = new ProxiedSolanaConnection('https://api.mainnet-beta.solana.com', { commitment: 'confirmed' }, './proxies.txt', 'TelegramCA-Validator');
     ...
     */
    async refreshActiveChatsMetadata() {
        try {
                ...
                for (const chatId of chatIds) {
                    try {
                        ...
                        if (result === null) {
                            ...
                            await execute('UPDATE telegram_monitored_chats SET is_active = 0 WHERE user_id = ? AND chat_id = ?', [userId, chatId]);
    ...
    async initialize() {
        ...
        try {
            ...
            const accounts = await queryAll('SELECT user_id, api_id, api_hash, phone_number, session_string FROM telegram_user_accounts WHERE is_verified = 1 AND session_string IS NOT NULL');
     ...
     */
    async startAuth(userId, apiId, apiHash, phoneNumber) {
        try {
            // Ensure telegram modules are loaded
            await ensureModulesLoaded();
            // Check if there's an existing session string in DB
            const existingSession = await queryOne('SELECT session_string FROM telegram_user_accounts WHERE user_id = ?', [userId]);
    ...
    async verifyCode(userId, code) {
        ...
        try {
            ...
            const user = result.user; // Api.User
            return {
                success: true,
                status: 'connected',
     ...
     * Verify 2FA password
     */
    async verify2FA(userId, password) {
        const session = this.authSessions.get(userId);
        if (!session || !session.client) {
            return {
                success: false,
        ...
        try {
            ...
            const passwordHash = await this.computePasswordHash(passwordInfo, password);
     ...
     */
    async computePasswordHash(passwordInfo, // Api.account.Password
    password) {
     ...
     */
    async preCacheChatEntities(client, chatId) {
        try {
     ...
     */
    async startMonitoring(userId, client) {
        ...
        client.addEventHandler(async (event) => {
            try {
                                ...
                                if (entityError.message?.includes('Could not find the input entity')) {
                                    ...
                                    try {
                                        ...
                                        if (messages && messages[0]) {
                                            ...
                                            const sender = fetchedMessage.sender;
                                            if (sender) {
                                                isBot = sender.bot || false;
                ...
                if (monitoredChat.monitoredUserIds && monitoredChat.monitoredUserIds.length > 0) {
                    const senderId = message.senderId?.toString();
                    const isFilteredUser = monitoredChat.monitoredUserIds.some((id) => id.toString() === senderId);
                    if (!isFilteredUser) {
                ...
                if (monitoredChat.monitoredKeywords && monitoredChat.monitoredKeywords.length > 0) {
                    matchedKeywords = monitoredChat.monitoredKeywords.filter((keyword) => message.message.toLowerCase().includes(keyword.toLowerCase()));
                ...
                if (contracts.length > 0) {
                    ...
                    for (const contract of contracts) {
                        ...
                        const shouldProcess = await this.shouldProcessContract(contract.address, chatId, message, chatConfig.duplicate_strategy);
                        ...
                        if (forwardDestinations.length > 0 && shouldProcess.forward) {
                            ...
                            for (const destination of forwardDestinations) {
                                ...
                                try {
                                    ...
                                    let forwardTarget = destination.target_chat_id;
     ...
     */
    async validateAndExtractToken(address) {
        ...
        try {
            ...
            const accountInfo = await this.solanaConnection.withProxy(conn => conn.getAccountInfo(publicKey));
            if (!accountInfo) {
     ...
     * Get sender username
     */
    async getSenderUsername(client, senderId) {
        try {
            if (!senderId)
                return undefined;
     ...
     */
    async fetchAndStoreChatMetadata(userId, chatId) {
        try {
            ...
            if (chatType === 'supergroup' || chatType === 'channel' || chatType === 'group') {
                try {
                    const fullChat = await telegramRateLimiter.executeCall('GetFullChannel', () => client.invoke(new Api.channels.GetFullChannel({
                        channel: chat
                    })), userId, 'normal');
                    ...
                    try {
                        const exportedInvite = await telegramRateLimiter.executeCall('ExportChatInvite', () => client.invoke(new Api.messages.ExportChatInvite({
                            peer: chat
                        })), userId, 'low');
                        if (exportedInvite && exportedInvite.link) {
                            metadata.inviteLink = exportedInvite.link;
                        }
                    }
                    catch (e) {
                    ...
                    try {
                        const me = await client.getMe();
                        const participant = await telegramRateLimiter.executeCall('GetParticipant', () => client.invoke(new Api.channels.GetParticipant({
                            channel: chat,
                            participant: me.id
                        })), userId, 'normal');
                ...
                try {
                    const fullChat = await telegramRateLimiter.executeCall('GetFullChat', () => client.invoke(new Api.messages.GetFullChat({
                        chatId: chat.id
                    })), userId, 'normal');
                    const fullChatInfo = fullChat.fullChat;
                    ...
                    const myParticipant = participants.find((p) => p.userId?.toString() === me.id.toString());
                    if (myParticipant) {
                        metadata.isMember = true;
            ...
            if (chatType === 'private') {
                try {
                    ...
                    const fullUser = await telegramRateLimiter.executeCall('GetFullUser', () => client.invoke(new Api.users.GetFullUser({
                        id: chat
                    })), userId, 'normal');
     ...
     */
    async leaveChatFromTelegram(userId, chatId) {
        try {
            const client = this.activeClients.get(userId);
            if (!client) {
     ...
     */
    async bulkLeaveChatsTelegram(userId, chatIds) {
        const successful = [];
        const failed = [];
        for (const chatId of chatIds) {
     ...
     */
    async fetchChatParticipants(userId, chatId, limit = 100) {
        try {
            const client = this.activeClients.get(userId);
            if (!client) {
            ...
            if (chat.className === 'Channel') {
                try {
                    const result = await telegramRateLimiter.executeCall('GetParticipants', () => client.invoke(new Api.channels.GetParticipants({
                        channel: chat,
     ...
     */
    async saveSession(userId, sessionString) {
        const encrypted = this.encrypt(sessionString);
        await execute('UPDATE telegram_user_accounts SET session_string = ?, is_verified = 1, last_connected_at = ?, updated_at = ? WHERE user_id = ?', [encrypted, Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000), userId]);
    }
    /**
     * Save comprehensive user profile data to database
     */
    async saveUserProfile(userId, client) {
        try {
     ...
     */
    async getMonitoredChats(userId) {
        const chats = await queryAll('SELECT chat_id, chat_name, is_active, monitored_keywords, monitored_user_ids, forward_to_chat_id, forward_account_id, process_bot_messages FROM telegram_monitored_chats WHERE user_id = ? AND is_active = 1', [userId]);
     ...
     */
    async cacheMessageToHistory(userId, chatId, message, client) {
        try {
     ...
     */
    async saveDetectedContract(userId, data) {
     ...
     */
    async logForwardingHistory(userId, data) {
        try {
            const now = Math.floor(Date.now() / 1000);
            await execute(`
     ...
     * Fetch user chats
     */
    async getUserChatsComprehensive(userId) {
        ...
        if (!client) {
            ...
            try {
                ...
                const account = await queryOne('SELECT user_id, api_id, api_hash, phone_number, session_string FROM telegram_user_accounts WHERE user_id = ? AND is_verified = 1 AND session_string IS NOT NULL', [userId]);
                if (!account) {
     ...
     */
    async disconnectAndCleanup(userId) {
     ...
     */
    async getForwardDestinations(userId, sourceChatId) {
    ...
    `, [userId, sourceChatId]);
      ...
      `, [userId, sourceChatId]);
            if (chat && chat.forward_to_chat_id) {
                return [{
                        target_chat_id: chat.forward_to_chat_id,
                        target_chat_name: chat.forward_to_chat_id,
                        forward_account_id: chat.forward_account_id
                    }];
            }
        }
        return destinations || [];
     ...
     * Get chat configuration for duplicate handling
     */
    async getChatConfig(userId, chatId) {
     ...
     */
    async shouldProcessContract(contractAddress, chatId, message, strategy) {
        ...
        switch (strategy) {
            ...
            case 'first_only_with_backlog':
                ...
                if (firstMention) {
                    ...
                    if (message.date < firstMention.message_timestamp) {
     ...
     */
    async markFirstMention(contractAddress, chatId, message) {
        const now = Math.floor(Date.now() / 1000);
        try {
            await execute(`
     ...
     */
    async detectContractsInMessage(messageText) {
     ...
     */
    async scanChatHistory(userId, chatId, depth = 1000) {
        const client = this.activeClients.get(userId);
        if (!client) {
     ...
     */
    async logTokenMint(_contractAddress, _data) {
        try {
     ...
     */
    async logTelegramDetection(data) {
        try {
            await execute(`
