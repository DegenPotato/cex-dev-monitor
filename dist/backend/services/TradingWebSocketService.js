...
class TradingWebSocketService {
     ...
     */
    async sendPortfolioUpdate(userId) {
        try {
     ...
     */
    async subscribeToWalletUpdates(userId, socket) {
        try {
            const walletManager = getWalletManager();
            const wallets = await walletManager.getUserWallets(userId);
     ...
     */
    async refreshWalletBalance(userId, walletId) {
        try {
            const walletManager = getWalletManager();
            // Verify ownership
     ...
     */
    async sendTradeUpdate(userId, trade, status, message) {
        const update = {
            type: 'trade_update',
            userId,
            data: {
...
// Singleton instance
let instance = null;
export function getTradingWebSocketService() {
    if (!instance) {
