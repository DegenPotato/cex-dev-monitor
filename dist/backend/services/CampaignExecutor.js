...
export class CampaignExecutor extends EventEmitter {
    ...
    async expireInstance(instanceId) {
        ...
        await execute(`UPDATE campaign_runtime_instances 
             ...
             WHERE id = ?`, [Math.floor(Date.now() / 1000), instanceId]);
    ...
    async createRuntimeInstance(campaign, triggerEvent) {
        const expiresAt = campaign.lifetime_ms
            ? Math.floor((Date.now() + campaign.lifetime_ms) / 1000)
            : null;
        const result = await execute(`INSERT INTO campaign_runtime_instances 
        ...
        await execute(`UPDATE campaigns 
             SET total_instances = total_instances + 1, 
                 last_activated_at = ? 
             WHERE id = ?`, [Math.floor(Date.now() / 1000), campaign.id]);
    ...
    async startCampaignExecution(instanceId, campaign, triggerNode) {
        if (!campaign.nodes)
            return;
    ...
    buildExecutionPlan(nodes, startNode) {
        const plan = [];
    ...
    async processNextNode(instanceId) {
    ...
    async executeNode(instanceId, node) {
        const instance = this.activeInstances.get(instanceId);
        if (!instance)
            return;
        ...
        await execute(`UPDATE campaign_runtime_instances SET current_node_id = ? WHERE id = ?`, [node.node_id, instanceId]);
        const step = {
        ...
        try {
            ...
            switch (node.node_type) {
                case 'filter':
                    result = await this.executeFilter(instance, node.config);
                    break;
                case 'monitor':
                    result = await this.executeMonitor(instance, node.config);
                    break;
                case 'action':
                    result = await this.executeAction(instance, node.config);
                    break;
                default:
            ...
            if (result.success === false) {
                ...
                await this.logEvent(instanceId, instance.campaign_id, node.node_id, node.node_type === 'filter' ? 'filter_passed' :
        ...
        await execute(`UPDATE campaign_runtime_instances 
             SET execution_log = ? 
             WHERE id = ?`, [JSON.stringify(instance.execution_log), instanceId]);
    }
    async executeFilter(instance, config) {
        // Get wallet info
    ...
    async executeMonitor(instance, config) {
        if (!instance.trigger_wallet) {
            return { success: true, skipped: true, reason: 'No wallet to monitor' };
        }
        // Register monitoring with SolanaEventDetector
        const detector = getSolanaEventDetector();
        await detector.addMonitoringTarget(instance.trigger_wallet, instance.campaign_id, instance.id, config);
    ...
    async executeAction(instance, config) {
    ...
    async executeWebhook(instance, config) {
        ...
        try {
            ...
            // Log webhook call
            const webhookId = await execute(`INSERT INTO campaign_webhook_calls 
                 (campaign_id, instance_id, node_id, webhook_url, payload)
                 VALUES (?, ?, ?, ?, ?)`, [instance.campaign_id, instance.id, 'action', config.webhook_url, payload]);
            // Make request
            ...
            await execute(`UPDATE campaign_webhook_calls 
                 SET response_status = ?, response_body = ? 
                 WHERE id = ?`, [response.status, JSON.stringify(response.data), webhookId.lastInsertRowid]);
            return {
                success: true,
    ...
    async executeTagDb(instance, config) {
        try {
            await execute(`INSERT INTO campaign_tags (campaign_id, instance_id, tag_type, tag_value, metadata)
                 VALUES (?, ?, ?, ?, ?)`, [
    ...
    async executeSendToFetcher(instance, config) {
    ...
    async executeCreateAlert(instance, config) {
        try {
            await execute(`INSERT INTO campaign_alerts 
    ...
    async handleNodeFailure(instanceId, node, error) {
        console.log(`❌ Node ${node.node_id} failed for instance ${instanceId}:`, error);
        // Log failure event
        await this.logEvent(instanceId, this.activeInstances.get(instanceId)?.campaign_id || 0, node.node_id, 'filter_failed', error);
    ...
    async handleMonitorDetected(data) {
    ...
    async failInstance(instanceId, error) {
        ...
        await execute(`UPDATE campaign_runtime_instances 
             ...
             WHERE id = ?`, [Math.floor(Date.now() / 1000), error, instanceId]);
        this.activeInstances.delete(instanceId);
        this.executionQueues.delete(instanceId);
        // Emit failure event
        this.emit('instance_failed', { instanceId, error });
    }
    async logEvent(instanceId, campaignId, nodeId, eventType, eventData) {
        await execute(`INSERT INTO campaign_events 
        ...
        if (metricField) {
            await execute(`INSERT INTO campaign_metrics (campaign_id, metric_date, ${metricField})
                 VALUES (?, ?, 1)
                 ON CONFLICT(campaign_id, metric_date) 
                 DO UPDATE SET ${metricField} = ${metricField} + 1`, [campaignId, metricDate]);
        }
    }
...
// Singleton instance
let campaignExecutor = null;
export function getCampaignExecutor() {
    if (!campaignExecutor) {
