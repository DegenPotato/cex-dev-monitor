...
export class ActivityBasedOHLCVCollector extends OHLCVCollector {
    constructor() {
        ...
        console.log('🚀 [OHLCV] Update tiers:', Object.entries(this.UPDATE_TIERS).map(([tier, ms]) => `${tier}: ${ms / 1000}s`).join(', '));
    }
    /**
    ...
    async discoverMissingPools() {
        try {
            ...
            const tokensWithoutPools = await queryAll(`SELECT DISTINCT r.token_mint as mint_address 
     ...
     */
    async checkPoolActivity() {
        try {
            console.log('🔍 [OHLCV] Checking pool activity...');
            // Get pools that need activity check
            const pools = await queryAll(`SELECT DISTINCT p.pool_address, p.mint_address, p.last_activity_check
         FROM token_pools p
     ...
     */
    async fetchAndUpdatePoolActivity(poolAddresses) {
        ...
        for (const poolData of data.data) {
            ...
            const tokenSettings = await queryOne(`SELECT ohlcv_realtime_enabled FROM token_registry WHERE token_mint = ?`, [poolData.id?.split('_')[1] || '']);
         ...
         SET activity_tier = ?, 
             ...
             last_activity_check = ?,
             next_update_at = ?
         WHERE pool_address = ?`, [tier, volume15m, volume1h, txns15m, now, now + nextUpdateMs, poolAddress]);
            // Also ensure pool exists in backfill progress (for compatibility)
            await execute(`INSERT OR IGNORE INTO ohlcv_backfill_progress 
         (pool_address, mint_address, timeframe, backfill_complete, oldest_timestamp, newest_timestamp)
         VALUES (?, ?, '15m', 0, 0, 0)`, [poolAddress, poolData.id?.split('_')[1] || '']);
            // Update or insert into schedule table
            await execute(`INSERT OR REPLACE INTO ohlcv_update_schedule 
     ...
     */
    async processRealtimeTier() {
        try {
            const now = Date.now();
            // Get pools due for real-time update
            const realtimePools = await queryAll(`SELECT s.pool_address, s.mint_address
         ...
         LIMIT 10`, [now]);
            ...
            for (const pool of realtimePools) {
                ...
                await execute(`UPDATE ohlcv_update_schedule 
           SET last_update = ?, next_update = ?
           WHERE pool_address = ?`, [now, now + this.UPDATE_TIERS.REALTIME, pool.pool_address]);
            }
            saveDatabase();
        }
        catch (error) {
    ...
    async processBackfillQueue() {
        const incompletePools = await queryAll(`SELECT DISTINCT p.mint_address, p.pool_address, r.first_seen_at * 1000 as creation_timestamp
       FROM token_pools p
     ...
     * Process pools based on their activity tiers
     */
    async processTieredUpdates() {
        const now = Date.now();
        // Get pools due for update (excluding REALTIME which has its own processor)
        const duePools = await queryAll(`SELECT pool_address, mint_address, update_tier
       ...
       LIMIT 50`, [now]);
        ...
        for (const pool of duePools) {
            ...
            const nextUpdateMs = this.UPDATE_TIERS[pool.update_tier]
                || this.UPDATE_TIERS.NORMAL;
            await execute(`UPDATE ohlcv_update_schedule 
         SET last_update = ?, next_update = ?
         WHERE pool_address = ?`, [now, now + nextUpdateMs, pool.pool_address]);
        }
        saveDatabase();
    }
    /**
     ...
     */
    async updatePoolOHLCV(poolAddress, mintAddress) {
        try {
            // Get the newest timestamp we have for each timeframe
            const timeframes = ['1m', '15m', '1h', '4h', '1d'];
            for (const timeframe of timeframes) {
                const progress = await queryOne(`SELECT newest_timestamp 
           ...
           WHERE pool_address = ? AND timeframe = ?`, [poolAddress, timeframe]);
                if (progress?.newest_timestamp) {
                    ...
                    await this.fetchLatestCandles(mintAddress, poolAddress, this.TIMEFRAMES.find(t => t.name === timeframe), progress.newest_timestamp);
                }
            }
        }
        catch (error) {
    ...
    async getStatus() {
        ...
        try {
            ...
            const tierStats = await queryAll(`SELECT 
          update_tier,
          COUNT(*) as count,
         ...
         GROUP BY update_tier`);
            ...
            const realtimeCount = await queryOne(`SELECT COUNT(*) as count FROM token_registry WHERE ohlcv_realtime_enabled = 1`);
            return {
                ...baseStatus,
