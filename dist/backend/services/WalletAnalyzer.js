...
export class WalletAnalyzer {
    constructor() {
        ...
        this.proxiedConnection = new ProxiedSolanaConnection('https://api.mainnet-beta.solana.com', { commitment: 'confirmed' }, './proxies.txt', 'WalletAnalyzer');
    ...
    async analyzeWallet(walletAddress) {
        ...
        try {
            ...
            let batch = await this.proxiedConnection.withProxy(conn => conn.getSignaturesForAddress(publicKey, { limit: 10 }));
            ...
            if (batch.length > 0) {
                ...
                if (batch.length === INITIAL_CHECK_LIMIT) {
                    ...
                    batch = await this.proxiedConnection.withProxy(conn => conn.getSignaturesForAddress(publicKey, { limit: 1000 }));
     ...
     */
    async quickFreshCheck(walletAddress) {
        try {
            const publicKey = new PublicKey(walletAddress);
            const signatures = await this.proxiedConnection.withProxy(conn => conn.getSignaturesForAddress(publicKey, { limit: 20 }));
     ...
     */
    async analyzeWallets(walletAddresses) {
        const results = new Map();
        // Analyze in parallel with rate limiting
        const batchSize = 5;
        for (let i = 0; i < walletAddresses.length; i += batchSize) {
            const batch = walletAddresses.slice(i, i + batchSize);
            const analyses = await Promise.all(batch.map(address => this.analyzeWallet(address)));
