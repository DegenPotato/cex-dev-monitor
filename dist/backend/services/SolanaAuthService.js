...
export class SolanaAuthService {
     ...
     */
    async generateAuthChallenge(walletAddress) {
        ...
        await this.challengeProvider.storeChallenge(walletAddress, nonce, message, expiresAt);
     ...
     */
    async verifySolanaSignature(walletAddress, signatureBase58, nonce) {
        try {
            ...
            if (!user) {
                ...
                await execute(`INSERT INTO users (solana_wallet_address, username, role, status, referral_code) 
                     VALUES (?, ?, 'user', 'active', ?)`, [walletAddress, username, referralCode]);
