...
class TelegramIntelligenceService {
     ...
     */
    async trackCaller(userId, caller) {
        try {
            // Check if caller already exists
            const existing = await queryOne('SELECT id FROM telegram_callers WHERE user_id = ? AND telegram_user_id = ?', [userId, caller.telegramUserId]);
            if (existing) {
        ...
        `, [
                ...
                const newCaller = await queryOne('SELECT id FROM telegram_callers WHERE user_id = ? AND telegram_user_id = ?', [userId, caller.telegramUserId]);
     ...
     */
    async recordTokenCall(userId, callerId, call) {
        try {
      ...
      `, [
            ...
            await execute('UPDATE telegram_callers SET total_calls = total_calls + 1, last_call_date = ? WHERE id = ?', [call.callTimestamp, callerId]);
      ...
      `, [userId, call.chatId, Date.now(), Date.now()]);
            ...
            setTimeout(() => {
                this.updateCallPerformance(result.id, call.contractAddress);
            }, 5 * 60 * 1000);
            return result.id;
        }
        catch (error) {
     ...
     */
    async updateCallPerformance(callId, contractAddress) {
        try {
            // Get original call data
            const call = await queryOne('SELECT price_at_call, mcap_at_call, call_timestamp, caller_id FROM telegram_token_calls WHERE id = ?', [callId]);
            if (!call)
                return;
            ...
            const priceAtCall = call.price_at_call || 0.000001;
      ...
      `, [
            ...
            await this.updateCallerMetrics(call.caller_id);
     ...
     */
    async updateCallerMetrics(callerId) {
        try {
      ...
      `, [callerId]);
            if (!calls)
                return;
            const winRate = (calls.successful / calls.total) * 100;
            // Calculate reputation score
            let reputationScore = 50; // Base score
            reputationScore += (winRate / 100) * 30; // Win rate factor
            reputationScore += Math.min(20, calls.total_volume / 1000000); // Volume factor
      ...
      `, [
                calls.successful || 0,
                calls.avg_multiplier || 0,
                calls.avg_time_to_peak || 0,
                calls.total_volume || 0,
                winRate || 0,
                reputationScore,
                trustLevel,
                Date.now(),
