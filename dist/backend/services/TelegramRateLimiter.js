...
export class TelegramRateLimiter extends EventEmitter {
    constructor() {
        ...
        // Rate limiting
        this.lastCallTime = 0;
        this.currentDelayMs = 500; // Start with 500ms between calls (more conservative)
     ...
     */
    async executeCall(method, callFn, userId, priority = 'normal') {
        ...
        try {
            ...
            apiTracker.trackCall('telegram', method, true, call.duration, 200, undefined);
            // Reduce delay on success (adaptive)
            if (this.consecutiveFloodErrors > 0) {
                this.consecutiveFloodErrors = 0;
                this.currentDelayMs = Math.max(this.config.baseDelayMs, this.currentDelayMs * 0.9);
            }
            return result;
        }
        catch (error) {
            ...
            if (error.errorMessage === 'FLOOD' || error.message?.includes('flood')) {
                ...
                this.currentDelayMs = Math.min(this.config.maxDelayMs, this.currentDelayMs * this.config.backoffMultiplier);
            ...
            const apiTracker = ApiProviderTracker.getInstance();
            apiTracker.trackCall('telegram', method, false, call.duration || 0, error.errorCode || 500, call.error);
            throw error;
        }
        finally {
            // Store call record
     ...
     */
    async waitForRateLimit(method, priority) {
        ...
        const priorityMultiplier = priority === 'high' ? 0.5 :
     ...
     */
    getMetrics() {
        const recentCalls = this.apiCalls.filter(c => c.timestamp > Date.now() - 60000);
     ...
     */
    adjustRateLimit(method, newLimit) {
