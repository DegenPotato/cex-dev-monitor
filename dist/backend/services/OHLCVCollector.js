...
export class OHLCVCollector {
    ...
    constructor() {
        ...
        this.BACKFILL_INTERVAL = 15 * 60 * 1000; // Run backfill every 15 minutes
        this.REQUESTS_PER_MINUTE = 30; // GeckoTerminal limit (for display only)
        this.MAX_CANDLES_PER_REQUEST = 1000; // GeckoTerminal max
        this.CANDLE_BATCH_SIZE = 100; // Incremental checkpoint interval
    ...
    async runBackfillCycle() {
        try {
            ...
            const tokens = await queryAll(`SELECT token_mint as mint_address, 
     ...
     */
    async processToken(mintAddress, creationTimestamp, forceRun = false) {
        try {
     ...
     */
    async ensurePoolAddresses(mintAddress) {
        ...
        const existing = await queryAll(`SELECT pool_address, dex, volume_24h_usd, is_primary 
       FROM token_pools 
       WHERE mint_address = ?
       ORDER BY is_primary DESC, volume_24h_usd DESC`, [mintAddress]);
        ...
        const tokenInfo = await queryOne(`SELECT migrated_pool_address, graduated_at FROM token_registry WHERE token_mint = ?`, [mintAddress]);
        ...
        try {
            ...
            pools.sort((a, b) => {
                ...
                if (migratedPoolAddress) {
                    if (a.pool_address === migratedPoolAddress)
                        return -1;
                    if (b.pool_address === migratedPoolAddress)
                        return 1;
                }
            ...
            for (let i = 0; i < pools.length; i++) {
                ...
                const existingPool = await queryOne(`SELECT id FROM token_pools WHERE mint_address = ? AND pool_address = ?`, [mintAddress, pool.pool_address]);
                if (existingPool) {
                    // Pool exists - UPDATE metadata only (preserve discovered_at)
                    await execute(`UPDATE token_pools 
             SET dex = ?, volume_24h_usd = ?, liquidity_usd = ?, price_usd = ?, is_primary = ?, last_verified = ?
             WHERE mint_address = ? AND pool_address = ?`, [pool.dex, pool.volume_24h_usd, pool.liquidity_usd, pool.price_usd, isPrimary, Date.now(), mintAddress, pool.pool_address]);
                    console.log(`♻️  [Dedup] Updated pool ${pool.pool_address.slice(0, 8)}... metadata`);
                }
                else {
                    // New pool - INSERT
                    await execute(`INSERT INTO token_pools 
             (mint_address, pool_address, dex, volume_24h_usd, liquidity_usd, price_usd, is_primary, discovered_at, last_verified) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [mintAddress, pool.pool_address, pool.dex, pool.volume_24h_usd, pool.liquidity_usd, pool.price_usd, isPrimary, Date.now(), Date.now()]);
            ...
            if (migratedPoolAddress && !pools.some(p => p.pool_address === migratedPoolAddress)) {
                ...
                const existingMigrated = await queryOne(`SELECT id FROM token_pools WHERE mint_address = ? AND pool_address = ?`, [mintAddress, migratedPoolAddress]);
                if (!existingMigrated) {
                    await execute(`INSERT INTO token_pools 
             (mint_address, pool_address, dex, volume_24h_usd, liquidity_usd, price_usd, is_primary, discovered_at, last_verified) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [mintAddress, migratedPoolAddress, 'raydium', 0, 0, 0, 1, Date.now(), Date.now()]);
     ...
     */
    async backfillTimeframe(mintAddress, poolAddress, timeframe, creationTimestamp) {
        ...
        try {
            ...
            const progress = await queryOne(`SELECT oldest_timestamp, newest_timestamp, backfill_complete 
         FROM ohlcv_backfill_progress 
         WHERE pool_address = ? AND timeframe = ?`, [poolAddress, timeframe.name]);
            ...
            const candles = await this.fetchOHLCV(poolAddress, timeframe, targetTimestamp);
            ...
            for (let i = 0; i < candles.length; i += this.CANDLE_BATCH_SIZE) {
                ...
                for (const candle of batch) {
                    try {
                        const result = await execute(`INSERT OR IGNORE INTO ohlcv_data 
            ...
            const errorProgress = await queryOne(`SELECT oldest_timestamp FROM ohlcv_backfill_progress WHERE pool_address = ? AND timeframe = ?`, [poolAddress, timeframe.name]);
            ...
            const currentProgress = await queryOne(`SELECT error_count FROM ohlcv_backfill_progress WHERE pool_address = ? AND timeframe = ?`, [poolAddress, timeframe.name]);
            const errorCount = (currentProgress?.error_count || 0) + 1;
            await execute(`UPDATE ohlcv_backfill_progress 
         SET error_count = ?, last_error = ?, last_fetch_at = ?
         WHERE pool_address = ? AND timeframe = ?`, [errorCount, error.message, Date.now(), poolAddress, timeframe.name]);
            saveDatabase();
     ...
     */
    async updateProgress(poolAddress, timeframe, mintAddress, data) {
        // Check if progress exists (DEDUPLICATION CHECK)
        const existing = await queryOne(`SELECT id, fetch_count FROM ohlcv_backfill_progress WHERE pool_address = ? AND timeframe = ?`, [poolAddress, timeframe]);
        if (existing) {
         ...
         WHERE pool_address = ? AND timeframe = ?`, [data.oldest, data.newest, data.complete ? 1 : 0, Date.now(), existing.fetch_count + 1, poolAddress, timeframe]);
        }
        else {
            // INSERT new progress
            await execute(`INSERT INTO ohlcv_backfill_progress 
         (mint_address, pool_address, timeframe, oldest_timestamp, newest_timestamp, backfill_complete, last_fetch_at, fetch_count)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [mintAddress, poolAddress, timeframe, data.oldest, data.newest, data.complete ? 1 : 0, Date.now(), 1]);
        }
    }
    /**
     * Fetch latest candles for a completed token (real-time updates)
     */
    async fetchLatestCandles(mintAddress, poolAddress, timeframe, lastTimestamp) {
        try {
            // Fetch candles that occurred AFTER our last stored timestamp
            const nowUnix = Math.floor(Date.now() / 1000);
            const candles = await this.fetchOHLCV(poolAddress, timeframe, nowUnix // Fetch up to now
            );
            if (candles.length === 0) {
                return;
            }
            ...
            for (const candle of newCandles) {
                try {
                    await execute(`INSERT OR IGNORE INTO ohlcv_data 
             (mint_address, pool_address, timeframe, timestamp, open, high, low, close, volume, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                        mintAddress,
                        poolAddress,
           ...
           WHERE pool_address = ? AND timeframe = ?`, [newestTimestamp, Date.now(), poolAddress, timeframe.name]);
                saveDatabase();
     ...
     */
    async fetchOHLCV(poolAddress, timeframe, beforeTimestamp) {
     ...
     */
    async markBackfillComplete(poolAddress, timeframe) {
        await execute(`UPDATE ohlcv_backfill_progress 
       SET backfill_complete = 1 
       WHERE pool_address = ? AND timeframe = ?`, [poolAddress, timeframe]);
        saveDatabase();
     ...
     */
    getPoolState(poolAddress, timeframe) {
    ...
    async getStatus() {
      ...
      `);
            ...
            const dedupStats = await queryOne(`
        SELECT 
