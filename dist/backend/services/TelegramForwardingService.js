...
export class TelegramForwardingService extends EventEmitter {
    constructor() {
        ...
        this.recentForwards = new Map(); // Rule ID -> recent attempts
        this.RATE_LIMIT_WINDOW_MS = 60 * 1000; // 1 minute
        this.HISTORY_RETENTION_DAYS = 7;
        this.entityCache = new Map(); // Cache resolved entities
        this.lastUserForwardTime = 0; // Track last user forward time
    ...
    async loadActiveRules() {
        try {
            const rules = await queryAll('SELECT * FROM telegram_forwarding_rules WHERE is_active = 1');
    ...
    canForward(rule) {
        ...
        recentAttempts = recentAttempts.filter(attempt => now - attempt.timestamp < 60 * 60 * 1000);
        this.recentForwards.set(ruleId, recentAttempts);
        // Check per-minute limit
        const attemptsLastMinute = recentAttempts.filter(attempt => now - attempt.timestamp < this.RATE_LIMIT_WINDOW_MS).length;
     ...
     */
    messagePassesFilters(rule, message) {
        // Filter by user ID
        ...
        if (rule.filterKeywords && rule.filterKeywords.length > 0 && message.text) {
            const hasKeyword = rule.filterKeywords.some(keyword => message.text.toLowerCase().includes(keyword.toLowerCase()));
            if (!hasKeyword)
                return false;
        }
     ...
     */
    async forwardMessage(sourceChatId, sourceAccountId, message, clientProvider // Function to get Telegram client
    ) {
        // Find applicable rules
        const applicableRules = Array.from(this.ruleCache.values()).filter(rule => rule.isActive &&
        ...
        for (const rule of applicableRules) {
            ...
            try {
                await telegramEntityCache.preloadEntities(await clientProvider(rule.targetAccountId), rule.targetChatIds);
            }
            catch (e) {
            ...
            for (const targetChatId of rule.targetChatIds) {
                ...
                let targetEntity; // Declare here so it's accessible in catch block
                try {
                    ...
                    if (isUserId) {
                        ...
                        if (this.entityCache.has(cacheKey)) {
                            ...
                            try {
                                ...
                                const userDialog = dialogs.find((d) => d.entity?.id?.toString() === targetChatId);
                                if (userDialog) {
                                ...
                                if (error.message?.includes('Could not find the input entity') ||
                                    ...
                                    throw new Error(`Cannot forward to user ${targetChatId}: No existing conversation found. ` +
                    ...
                    apiProviderTracker.trackCall('telegram', rule.forwardMode === 'forward' ? 'forwardMessages' : 'sendMessage', true, responseTime, 200);
                    // Log success
                    await this.logForwardingHistory(rule.id, rule.userId, sourceChatId, message, targetChatId, 'success', null, responseTime);
                    ...
                    if (isUserId) {
                        ...
                        try {
                            ...
                            client.getDialogs({ limit: 10 }).catch((e) => {
                    ...
                    if (error.message?.includes('A wait of') && error.message?.includes('seconds is required')) {
                        ...
                        if (waitMatch) {
                            ...
                            try {
                                ...
                                // Log success
                                await this.logForwardingHistory(rule.id, rule.userId, sourceChatId, message, targetChatId, 'success', `Succeeded after ${waitSeconds}s rate limit wait`, retryResponseTime);
                                // Track forward attempt for rate limiting
                                this.trackForwardAttempt(rule.id);
                                // Update rule stats
                                await this.updateRuleStats(rule.id, true);
                    ...
                    apiProviderTracker.trackCall('telegram', rule.forwardMode === 'forward' ? 'forwardMessages' : 'sendMessage', false, responseTime, error.errorMessage === 'FLOOD' ? 429 : 500, error.message);
                    // Log failure
                    await this.logForwardingHistory(rule.id, rule.userId, sourceChatId, message, targetChatId, 'failed', error.message, responseTime);
                    // Update rule stats
     ...
     */
    async logForwardingHistory(ruleId, userId, sourceChatId, message, targetChatId, status, errorMessage, responseTimeMs) {
        const now = Math.floor(Date.now() / 1000);
        await execute(`
    ...
    async cleanupOldHistory() {
        ...
        try {
            await execute('DELETE FROM telegram_forwarding_history WHERE forwarded_at < ?', [cutoffTime]);
    ...
    async createRule(userId, rule) {
      ...
      `, [
            ...
            const result = await queryOne(`SELECT id FROM telegram_forwarding_rules 
         WHERE user_id = ? AND rule_name = ? 
         ORDER BY id DESC LIMIT 1`, [userId, rule.ruleName || 'Untitled Rule']);
    ...
    async getUserRules(userId) {
        const rules = await queryAll('SELECT * FROM telegram_forwarding_rules WHERE user_id = ? ORDER BY created_at DESC', [userId]);
    ...
    async deleteRule(userId, ruleId) {
        try {
            await execute('DELETE FROM telegram_forwarding_rules WHERE id = ? AND user_id = ?', [ruleId, userId]);
     ...
     */
    async toggleRule(userId, ruleId, isActive) {
        try {
            const now = Math.floor(Date.now() / 1000);
            await execute('UPDATE telegram_forwarding_rules SET is_active = ?, updated_at = ? WHERE id = ? AND user_id = ?', [isActive ? 1 : 0, now, ruleId, userId]);
            await this.loadActiveRules(); // Refresh cache
            return { success: true };
        }
        catch (error) {
