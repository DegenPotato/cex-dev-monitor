 ...
 */
router.post('/account/save', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const { google_user_id, email, access_token, refresh_token, expires_in, // seconds
        scope, } = req.body;
 ...
 */
router.get('/account', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 */
router.post('/account/refresh', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 */
router.post('/account/revoke', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const account = await YouTubeAccountProvider.getAccountByUserId(userId);
        if (!account) {
 ...
 */
router.post('/playlists/cache', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 */
router.get('/playlists/cache', authService.requireSecureAuth(), async (req, res) => {
    try {
        const userId = req.user.id;
        const account = await YouTubeAccountProvider.getAccountByUserId(userId);
        if (!account) {
