...
export class SolPriceOracle {
    constructor() {
        ...
        this.GECKOTERMINAL_API = 'https://api.geckoterminal.com/api/v2/simple/networks/solana/token_price';
        this.SOL_ADDRESS = 'So11111111111111111111111111111111111111112';
        this.POLL_INTERVAL = 30000; // 30 seconds
        this.currentPrice = 150; // Fallback default
        this.lastUpdate = 0;
    }
    /**
    ...
    async fetchPrice() {
        ...
        try {
            const response = await fetch(`${this.GECKOTERMINAL_API}/${this.SOL_ADDRESS}`, { headers: { 'Accept': 'application/json' } });
            if (!response.ok) {
     ...
     */
    async updatePriceValue(newPrice) {
