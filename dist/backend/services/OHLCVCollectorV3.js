...
export class OHLCVCollectorV3 {
    ...
    async processToken(token) {
        ...
        for (const pool of pools) {
            ...
            for (const timeframe of this.TIMEFRAMES) {
                await this.collectTimeframe(mintAddress, pool, timeframe, token.timestamp, token.graduated_at);
            }
        }
    }
    /**
     ...
     */
    async discoverAllPools(mintAddress, migratedPoolAddress) {
        const pools = [];
    ...
    detectPoolType(dex, poolAddress) {
    ...
    async fetchPoolsFromGecko(mintAddress) {
     ...
     */
    async collectTimeframe(mintAddress, pool, timeframe, creationTimestampMs, migrationTimestampMs) {
        // Check progress
    ...
    `, [mintAddress, pool.pool_address, timeframe.name]);
        ...
        if (candles.length === 0) {
            ...
            if (fetchMode === 'initial') {
                await this.updateProgress(mintAddress, pool.pool_address, timeframe.name, nowUnix, nowUnix, true, pool.pool_type, migrationUnix);
            }
            return;
        }
        ...
        if (stored > 0) {
            ...
            await this.updateProgress(mintAddress, pool.pool_address, timeframe.name, newOldest, newNewest, isComplete, pool.pool_type, migrationUnix);
        }
    }
    /**
    ...
    async updateProgress(mintAddress, poolAddress, timeframe, oldestTimestamp, newestTimestamp, complete, poolType, migrationTimestamp) {
        await execute(`
