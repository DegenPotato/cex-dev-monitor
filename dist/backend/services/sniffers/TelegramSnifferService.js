import { BaseSnifferService, SnifferPlatform } from './BaseSnifferService.js';
...
export class TelegramSnifferService extends BaseSnifferService {
     ...
     */
    async fetchAvailableChats(_options) {
        try {
            ...
            const unifiedChats = telegramChats.map((chat) => this.transformToUnifiedFormat(chat));
            return unifiedChats;
        }
        catch (error) {
     ...
     */
    transformToUnifiedFormat(telegramChat) {
     ...
     */
    async startMonitoring(chatId, config) {
     ...
     */
    async forwardContent(_content, config) {
     ...
     */
    async analyzeContent(content, _config) {
