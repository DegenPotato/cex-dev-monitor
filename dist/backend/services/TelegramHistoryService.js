...
export class TelegramHistoryService {
     ...
     */
    async fetchAndStoreChatHistory(userId, chatId, limit = 1000, onProgress) {
        try {
            ...
            const chatInfo = await queryOne('SELECT chat_type FROM telegram_monitored_chats WHERE user_id = ? AND chat_id = ?', [userId, chatId]);
            ...
            const fetchStatus = await queryOne('SELECT * FROM telegram_chat_fetch_status WHERE user_id = ? AND chat_id = ?', [userId, chatId]);
            let messages = [];
            let offsetId = 0;
            let hasMore = true;
            let apiCalls = 0;
     ...
     */
    async getCachedHistory(userId, chatId, limit = 100, offset = 0) {
     ...
     */
    async getFetchStatus(userId, chatId) {
        return await queryOne('SELECT * FROM telegram_chat_fetch_status WHERE user_id = ? AND chat_id = ?', [userId, chatId]);
    }
    /**
     ...
     */
    async deleteHistory(userId, chatId) {
        try {
