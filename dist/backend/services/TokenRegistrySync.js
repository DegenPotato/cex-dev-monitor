...
class TokenRegistrySyncService {
    ...
    async syncTokens() {
        try {
            ...
            const missingTokens = await queryAll(`
        SELECT DISTINCT 
          tm.mint_address,
     ...
     */
    async addTokenToRegistry(token) {
        try {
            await execute(`
     ...
     */
    async getTokensWithPricing(limit = 100, offset = 0) {
        try {
     ...
     */
    async getLaunchMetrics(mintAddress) {
        try {
            // Get token registry entry to find first_seen_at timestamp
            const registryEntry = await queryOne(`
        SELECT first_seen_at
        FROM token_registry
        WHERE token_mint = ?
      `, [mintAddress]);
            // Get earliest OHLCV data (launch price)
            const launchData = await queryOne(`
      ...
      `, [mintAddress]);
            ...
            if (registryEntry?.first_seen_at) {
                const firstSeenData = await queryOne(`
    ...
    async getAnalytics() {
      ...
      `);
            const tokens24h = await queryOne(`
        SELECT COUNT(*) as count FROM token_registry
        WHERE first_seen_at > strftime('%s', 'now', '-1 day') * 1000
      `);
            const tokens7d = await queryOne(`
        SELECT COUNT(*) as count FROM token_registry
