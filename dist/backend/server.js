...
app.post('/api/wallets/test-dev', async (req, res) => {
    ...
    (async () => {
        try {
            ...
            const signatures = await proxiedConnection.withProxy(conn => conn.getSignaturesForAddress(pubkey, { limit: 10 }));
...
app.post('/api/ohlcv/toggle-realtime/:mintAddress', async (req, res) => {
    try {
        ...
        await execute(`UPDATE token_mints 
       SET ohlcv_realtime_enabled = ?, ohlcv_update_tier = ?
       WHERE mint_address = ?`, [enabled ? 1 : 0, enabled ? 'REALTIME' : 'NORMAL', mintAddress]);
        // Update schedule if pools exist
        await execute(`UPDATE ohlcv_update_schedule 
       SET update_tier = ?, next_update = ?
       WHERE mint_address = ?`, [enabled ? 'REALTIME' : 'NORMAL', Date.now() + (enabled ? 60000 : 900000), mintAddress]);
        saveDatabase();
        res.json({
            success: true,
            mintAddress,
...
app.get('/api/ohlcv/:address/:timeframe', async (req, res) => {
    try {
        ...
        if (timeframe === '1m') {
            const rawCandles = await queryAll(`SELECT pool_address, timestamp, open, high, low, close, volume 
         FROM ohlcv_data 
         ...
         LIMIT ?`, [address, timeframe, limit]);
            // Additional validation to ensure no null values
            candles = rawCandles.filter((c) => c && c.timestamp && c.open && c.high && c.low && c.close &&
            ...
            candles = await ohlcvAggregator.aggregateCandles(address, null, // Will aggregate across all pools
            timeframe, limit);
        }
        // Get token metadata (including migration info)
        const tokenMeta = await queryOne(`SELECT launchpad_completed_at, migrated_pool_address 
       FROM token_mints 
       WHERE mint_address = ?`, [address]);
        // Get all pools for this token
        const pools = await queryAll(`SELECT pool_address, dex, volume_24h_usd, is_primary 
       FROM token_pools 
       WHERE mint_address = ?
...
app.post('/api/ohlcv/fetch-test/:mintAddress', authService.requireSuperAdmin(), async (req, res) => {
    ...
    try {
        ...
        const pools = await queryAll(`SELECT pool_address, dex FROM token_pools WHERE mint_address = ? ORDER BY is_primary DESC`, [mintAddress]);
        if (pools.length === 0) {
        ...
        const candles = ohlcvArray.map((candle) => ({
            timestamp: candle[0],
...
app.post('/api/ohlcv/test/:mintAddress', authService.requireSuperAdmin(), async (req, res) => {
    ...
    try {
        ...
        // Get token info
        const tokenInfo = await queryOne(`SELECT timestamp, name, symbol FROM token_mints WHERE mint_address = ?`, [mintAddress]);
        if (!tokenInfo) {
...
app.get('/api/ohlcv/test-status/:mintAddress', async (req, res) => {
    ...
    `, [mintAddress]);
      ...
      WHERE mint_address = ?
    `, [mintAddress]);
        const candleCounts = await queryAll(`
      SELECT 
        pool_address,
        timeframe,
        COUNT(*) as count,
    ...
    `, [mintAddress]);
        const summary = {
            ...
            totalCandles: candleCounts?.reduce((sum, c) => sum + (c.count || 0), 0) || 0,
            completedTimeframes: progress?.filter((p) => p.backfill_complete)?.length || 0,
...
app.get('/api/stats/requests', (_req, res) => {
    ...
    for (const [provider, providerData] of Object.entries(apiProviderStats)) {
        ...
        mergedEndpointRates[displayName] = providerData.callsLastMinute || 0;
        mergedByEndpoint[displayName] = providerData.totalCalls || 0;
    }
    // Merge service rates AND totals
    const mergedServiceRates = { ...rpcStats.serviceRates };
    const mergedByService = { ...rpcStats.byService };
    for (const [provider, providerData] of Object.entries(apiProviderStats)) {
        mergedServiceRates[provider] = providerData.callsLastMinute || 0;
        mergedByService[provider] = providerData.totalCalls || 0; // FIX: Add total calls
    }
...
app.get('/api/market-data/analyze-mint/:mintAddress', async (req, res) => {
    try {
        ...
        const bondingCurveAccounts = tx.transaction.message.accountKeys.filter((key) => key.pubkey.toBase58().includes('pump') || key.signer === false);
        res.json({
            mintAddress,
            createSignature: createSig,
            blockTime: tx.blockTime,
            accounts: bondingCurveAccounts.map((acc) => ({
