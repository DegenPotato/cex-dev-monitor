...
async function testTimestamps() {
    ...
    const tokens = await queryAll(`SELECT mint_address, timestamp 
     FROM token_mints 
    ...
    const progress = await queryAll(`SELECT pool_address, oldest_timestamp, newest_timestamp
    ...
    const candles = await queryAll(`SELECT timestamp, pool_address
     FROM ohlcv_data
     ORDER BY timestamp DESC
     LIMIT 3`);
    if (candles.length > 0) {
