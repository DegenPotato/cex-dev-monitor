...
// Discriminators from SmartMoneyTracker (proven to work)
...
// Test configuration
const TEST_TOKEN = '48WduMWSJejhk48PUFXdeXHEWXK7wq8KYb6TupvqHuwe';
const TIMEFRAME_MINUTES = 5;
const LOOKBACK_HOURS = 24; // Increased to 24 hours to see if token has any activity

console.log('🚀 Testing Onchain OHLCV Builder\n');
console.log(`Token: ${TEST_TOKEN}`);
console.log(`Timeframe: ${TIMEFRAME_MINUTES}m`);
console.log(`Lookback: ${LOOKBACK_HOURS}h\n`);

const connection = new Connection('https://tritono-main-e861.mainnet.rpcpool.com/00d87746-cade-4061-b5cf-5e4fc1deab03');

async function testOHLCVBuilder() {
  const startTime = Date.now();
  
  try {
    console.log('📡 Step 1: Fetching signatures...');
    ...
    for (let i = 0; i < signatures.length; i += batchSize) {
      ...
      for (const tx of transactions) {
        ...
        const swapData = parseSwapTransaction(tx, TEST_TOKEN);
        if (swapData) {
          swaps.push(swapData);
        }
      }
...
function generateHTMLReport(candles, metadata) {
